...
<details>
<summary style="margin-left: 40px">2.1 - Are you using a Digital Business Automation on Cloud Trial or Demo Environment?</summary>
...
1. Optionally, add all adinistrator roles as appropriate (below for reference).
...
<details>
<summary style="margin-left: 40px">2.2 - Are You Using Your Own Environment (not an on Cloud trial/demo)?</summary>
...
As a Line of Business Supervisor, you decide to create a lightweight workflow appllication to manage incoming requests and avoid a disorganized email inbox.
...
2. If you see **Create workstreams** you have been given the required access to continue (Note, if you do not see this menu option, close the menu, wait a 5-10 seconds and try again)
...
6. Scroll down on the right side to configure your workflow's first activity
    ...
    1. Activate the data items clider, click **Select data items**, click the plus sign and add two data item as pictured below.
...
7. Review the workstream and note the **Read-only** notice next to the name since you are only revewing, not editing.
